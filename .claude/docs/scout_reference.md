...
- **Use an independent validator model.** Validate with a different model from the one doing the scanning to reduce shared biases (see Validation above). Validators agree closely when the signal is explicit and diverge at definitional boundaries — those divergences tell you where the construct is fuzzy.
