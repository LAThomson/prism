...
- **Ceiling and floor effects are non-findings for between-condition comparison, and must be tagged on every per-condition rate.** A signal at 100% (or 0%) in every condition cannot discriminate them; say so explicitly rather than reporting the invariant rate as a result. Per `analyst_interface_contract.md §Quantified Results`, append `[floor]` to any per-condition rate whose Wilson 95% CI upper bound is below 0.20, and `[ceiling]` when the lower bound is above 0.80 — the trigger is the CI, not the point estimate, so suggestive cases (e.g. a 12% rate with upper CI 18%) are tagged alongside the obvious ones. Surface these markers in the Summary as well as §Quantified Results — they are the trigger for the orchestrator's Step 2h headroom gate, and the orchestrator cannot run that gate on rates the analyst left untagged.
